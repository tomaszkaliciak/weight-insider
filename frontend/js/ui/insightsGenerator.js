...
export const InsightsGenerator = {
  ...
  _getConsistencyStatus(consistencyWgt, consistencyCal) {
    ...
    if (wgtPct == null || calPct == null || isNaN(wgtPct) || isNaN(calPct)) { // Added NaN checks
  ...
  _getTrendStatus(currentTrendWeekly, currentWeight, regressionUsed) {
    ...
    const trendPercent = (weightForPct != null && weightForPct > 0 && !isNaN(weightForPct))
        ? (currentTrendWeekly / weightForPct) * 100 : null;
    const trendValStr = `<strong>${fv(currentTrendWeekly, 2)} kg/wk</strong>`;
    const trendPercentStr = trendPercent != null ? ` (${fv(trendPercent, 1)}% BW)` : ""; // Abbreviate
    ...
    } else if (currentTrendWeekly > 0) { // Gaining
   ...
   */
  _getGoalStatus(displayStats, isGoalAchieved) { // Use isGoalAchieved flag
    ...
    if (isGoalAchieved) { // Use flag
        ...
        const weightDiffClass = displayStats.weightToGoal >= 0 ? "gaining" : "losing";
    ...
    const isMeaningfulTime = timeEstimate && !["N/A", "Goal Achieved!", "Trending away", "Trend flat"].includes(timeEstimate);

    if (isMeaningfulTime && !isGoalAchieved) { // Check flag
        status += ` Est. <span class="good">${timeEstimate}.</span>`;
    } else if ((timeEstimate === "Trending away" || timeEstimate === "Trend flat") && !isGoalAchieved) { // Check flag
  ...
  _getDetectedFeaturesInsightHTML(stateSnapshot) {
    ...
    if (!(analysisRange.start instanceof Date) || !(analysisRange.end instanceof Date) || isNaN(analysisRange.start) || isNaN(analysisRange.end)) return "";
    ...
    const plateausInRange = (Array.isArray(plateaus) ? plateaus : []).filter(p =>
        p.endDate instanceof Date && p.startDate instanceof Date &&
        p.endDate >= analysisStartDate && p.startDate <= analysisEndDate
    );
    const changesInRange = (Array.isArray(trendChangePoints) ? trendChangePoints : []).filter(p =>
        p.date instanceof Date &&
        p.date >= analysisStartDate && p.date <= analysisEndDate
    );
    ...
    if (changesInRange.length > 0) {
        ...
        changesInRange.sort((a, b) => a.date - b.date).forEach(p => {
  ...
  _renderSummary(displayStats) {
    ...
    if (!ui.insightSummaryContainer || ui.insightSummaryContainer.empty()) {
        if (!this._warnedContainerMissing) {
            console.warn("InsightsGenerator: Insight summary container (#insight-summary) not found in uiCache.");
    ...
    if (!displayStats || typeof displayStats !== 'object') {
        console.warn("InsightsGenerator: Invalid displayStats received in _renderSummary.");
    ...
    const currentTrendWeekly = displayStats.regressionSlopeWeekly ?? displayStats.currentWeeklyRate;
    const regressionUsedForTrend = displayStats.regressionSlopeWeekly != null;

    let summaryHtml = "";
    try {
        // Generate individual insight components
        const trendStatus = this._getTrendStatus(currentTrendWeekly, displayStats.currentSma, regressionUsedForTrend);
        const tdeeStatus = this._getPrimaryTDEEStatus(displayStats.avgExpenditureGFit, displayStats.avgTDEE_WgtChange, displayStats.avgTDEE_Adaptive);
        const goalStatus = this._getGoalStatus(displayStats, isGoalAchieved);
        const consistencyStatus = this._getConsistencyStatus(displayStats.weightDataConsistency, displayStats.calorieDataConsistency);
        const detectedFeaturesHtml = this._getDetectedFeaturesInsightHTML(stateSnapshot); // Pass full state
        ...
        console.error("InsightsGenerator: Error generating summary box HTML", error);
        summaryHtml = "<p class='error'>Error generating summary. Check console.</p>";
    }

    // Update the container's content
    ui.insightSummaryContainer.html(summaryHtml || "<p>Analysis requires more data or a different range.</p>");
  },
   ...
   */
  init() {
    StateManager.subscribeToSpecificEvent('state:displayStatsUpdated', (payload) => this._renderSummary(payload)); // Payload *is* displayStats
    console.log("[InsightsGenerator Init] Subscribed to state:displayStatsUpdated.");
