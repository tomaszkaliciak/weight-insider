...
import { EventHandlers } from "../interactions/eventHandlers.js"; // For attaching event handlers
import { Utils } from "../core/utils.js"; // For formatting or utilities if needed
// DataService import removed - trend calculation should happen upstream.
...
export const FocusChartUpdater = {
    ...
    updateAxes(focusWidth, focusHeight, options = {}) {
        if (!focusWidth || !focusHeight || focusWidth <= 0 || focusHeight <= 0 || !axes.xAxis || !axes.yAxis || !scales.x || !scales.y) {
            console.warn("FocusChartUpdater: Axes/scales not initialized or invalid dimensions for axes update.");
            return;
        }
        ...
        const yTicks = axes.yAxis.scale()?.ticks ? axes.yAxis.scale().ticks(axes.yAxis.ticks()[0]) : [];
        const yGridAxis = d3.axisLeft(scales.y).tickSize(-focusWidth).tickFormat("").tickValues(yTicks);

        const gridUpdate = ui.gridGroup;
        if (dur > 0 && !options.isInteractive) { // Only transition grid if not interactive
    ...
    updatePaths(
        ...
        const lineGenFactory = (yAccessor) => d3.line()
            .x(d => currentXScale(d.date))
            .y(d => currentYScale(yAccessor(d)))
            .curve(d3.curveMonotoneX)
            .defined(d => d.date instanceof Date && !isNaN(d.date) && yAccessor(d) != null && isFinite(currentYScale(yAccessor(d))) && isFinite(currentXScale(d.date)));

        const areaGenFactory = (y0Accessor, y1Accessor) => d3.area()
            .x(d => currentXScale(d.date))
            .y0(d => currentYScale(y0Accessor(d)))
            .y1(d => currentYScale(y1Accessor(d)))
            .curve(d3.curveMonotoneX)
            .defined(d => d.date instanceof Date && !isNaN(d.date) && y0Accessor(d) != null && y1Accessor(d) != null && isFinite(currentYScale(y0Accessor(d))) && isFinite(currentYScale(y1Accessor(d))) && isFinite(currentXScale(d.date)));
        ...
        const updateSelection = (selection, pathData, generator) => {
            ...
            const pathString = (pathData && pathData.length > 0) ? generator(pathData) : ""; // Generate path string if data exists
     ...
     * @param {boolean} [options.isInteractive=false] - If true, use faster/no transitions.
     */
    updateDots(visibleRawWeightData, pinnedTooltipData = null, activeHoverData = null, options = {}) {
        ...
        const rawDotsDataValid = visibleRawWeightData.filter(d =>
            d.value != null && d.date instanceof Date && !isNaN(d.date) &&
            isFinite(scales.x(d.date)) && isFinite(scales.y(d.value))
        );
        ...
        rawDots.join(
            enter => enter.append("circle")
                ...
                .call(enter => enter.transition().duration(dur).style("opacity", 0.4)), // Fade in
            update => update
                // Ensure handlers are attached on update too (redundant but safe)
                .on("mouseover", EventHandlers.dotMouseOver)
                .on("mouseout", EventHandlers.dotMouseOut)
                .on("click", EventHandlers.dotClick)
                .call(update => update.transition().duration(dur) // Transition existing dots
                    .attr("cx", d => scales.x(d.date))
                    .attr("cy", d => scales.y(d.value))
                    .attr("r", d => { // Update radius based on hover/pin state passed in
                        const isPinned = pinnedTooltipData?.id === d.date?.getTime();
                        const isHovered = activeHoverData?.date?.getTime() === d.date?.getTime();
                        return (isHovered || isPinned) ? CONFIG.dotHoverRadius : CONFIG.rawDotRadius;
                    })
                    .style("opacity", d => { // Update opacity based on hover/pin
                        const isPinned = pinnedTooltipData?.id === d.date?.getTime();
                        const isHovered = activeHoverData?.date?.getTime() === d.date?.getTime();
            ...
            exit => exit
                ...
                .transition().duration(dur).style("opacity", 0).remove() // Fade out
        );
    },
    ...
    updateHighlightMarker(highlightedDate, visibleRawWeightData) {
        ...
        const highlightDataPoint = highlightedDate
            ? visibleRawWeightData.find(d =>
                d.value != null && d.date instanceof Date && d.date.getTime() === highlightedDate.getTime() &&
                isFinite(scales.x(d.date)) && isFinite(scales.y(d.value)) // Check if plottable
            )
            : null;

        const highlightMarker = ui.highlightGroup
            .selectAll(".highlight-marker")
            .data(highlightDataPoint ? [highlightDataPoint] : [], d => d.date.getTime()); // Use date as key
        ...
        highlightMarker.join(
            enter => enter.append("circle")
                ...
                .style("opacity", 0)
                .call(enter => enter.transition().duration(dur) // Transition in
                    .attr("r", CONFIG.dotRadius * CONFIG.highlightRadiusMultiplier)
                    .style("opacity", 0.8)),
            update => update
                .transition().duration(dur) // Transition existing marker
                .attr("cx", d => scales.x(d.date))
                .attr("cy", d => scales.y(d.value))
                .attr("r", CONFIG.dotRadius * CONFIG.highlightRadiusMultiplier)
                .style("opacity", 0.8), // Ensure it stays visible
            exit => exit
                .transition().duration(dur) // Transition out
                .attr("r", 0)
                .style("opacity", 0)
                .remove()
        );
    ...
    updateCrosshair(hoverData, focusWidth, focusHeight) {
        if (!ui.crosshairGroup || !scales.x || !scales.y || !focusWidth || !focusHeight) {
        ...
        if (yPos != null && isFinite(xPos) && isFinite(yPos) &&
            xPos >= 0 && xPos <= focusWidth &&
            yPos >= 0 && yPos <= focusHeight) {
            ui.crosshairGroup.style("display", null); // Show group
            ui.crosshairGroup.select(".crosshair.crosshair-x")
                .attr("y1", 0).attr("y2", focusHeight)
                .attr("transform", `translate(${xPos}, 0)`);
            ui.crosshairGroup.select(".crosshair.crosshair-y")
    ...
    updateAnnotations(annotations, processedData, options = {}) {
        ...
        if (!ui.annotationsGroup || !scales.x || !scales.y) {
            console.warn("FocusChartUpdater: Cannot update annotations - group or scales missing.");
            return;
        }
        ...
        const visibleAnnotations = annotations.filter(a => {
            ...
            return !isNaN(date.getTime()) && date >= xDomain[0] && date <= xDomain[1] && a.type === 'point';
        });
        ...
        const findYValue = (targetDate) => {
            ...
            const yVal = closestPoint ? (closestPoint.sma ?? closestPoint.value) : null;
        ...
        markers.join(
            enter => {
                ...
                group.attr("transform", d => { // Set initial position
                    const yValue = findYValue(new Date(d.date + 'T00:00:00'));
                    const xPos = scales.x(new Date(d.date + 'T00:00:00'));
                    return (yValue != null && isFinite(xPos)) ? `translate(${xPos}, ${scales.y(yValue)})` : `translate(-1000, -1000)`;
                });

                group.append("circle") // Append the marker shape
                ...
                group.on("mouseover", EventHandlers.annotationMouseOver) // Attach handlers
            ...
            update => update
                ...
                .on("mouseout", EventHandlers.annotationMouseOut)
                .transition().duration(dur) // Transition to new position
                .style("opacity", 0.8)
                .attr("transform", d => {
                    const yValue = findYValue(new Date(d.date + 'T00:00:00'));
                    const xPos = scales.x(new Date(d.date + 'T00:00:00'));
            ...
            exit => exit
                .transition().duration(dur / 2).style("opacity", 0) // Transition out
    ...
    */
    updatePlateauRegions(plateaus, focusHeight, options = {}) {
        ...
        const xDomain = scales.x.domain();
        const visiblePlateaus = plateaus.filter(p =>
            p.endDate instanceof Date && p.startDate instanceof Date &&
            p.endDate >= xDomain[0] && p.startDate <= xDomain[1]
        );

        const regions = ui.plateauGroup
            .selectAll(".plateau-region")
            .data(visiblePlateaus, d => `${d.startDate.toISOString()}-${d.endDate.toISOString()}`);
        ...
        regions.join(
            enter => enter.append("rect")
                ...
                .style("pointer-events", "none")
                .style("opacity", 0)
                .call(enter => enter.transition().duration(dur).style("opacity", 0.25)),
            update => update
    ...
    */
    updateTrendChangeMarkers(trendChangePoints, processedData, options = {}) {
        ...
        const xDomain = scales.x.domain();
        const visibleMarkersData = trendChangePoints.filter(p =>
            p.date instanceof Date && !isNaN(p.date) &&
            p.date >= xDomain[0] && p.date <= xDomain[1]
        );

        const findYValue = (targetDate) => { /* ... (same as in updateAnnotations) ... */
            if (!(targetDate instanceof Date) || isNaN(targetDate.getTime())) return null;
            const targetTime = targetDate.getTime();
            let closestPoint = null; let minDiff = Infinity;
            processedData.forEach(d => { if (d.date instanceof Date) { const diff = Math.abs(d.date.getTime() - targetTime); if (diff < minDiff) { minDiff = diff; closestPoint = d; } } });
            const yVal = closestPoint ? (closestPoint.sma ?? closestPoint.value) : null;
            return (yVal != null && isFinite(scales.y(yVal))) ? yVal : null;
        };

        const markerSize = 4;
        const markerPath = d3.symbol().type(d3.symbolTriangle).size(markerSize * markerSize * 1.5);
        ...
        markers.join(
            enter => { /* ... (same as before) ... */
                const group = enter.append("g").attr("class", "trend-change-marker-group").style("opacity", 0);
                group.attr("transform", d => {
                    const yValue = findYValue(d.date); const rotation = d.magnitude > 0 ? 180 : 0; const xPos = scales.x(d.date); const yPos = (yValue != null) ? scales.y(yValue) : null;
                    return (xPos != null && yPos != null && isFinite(xPos) && isFinite(yPos)) ? `translate(${xPos}, ${yPos}) rotate(${rotation})` : `translate(-1000, -1000)`;
                });
                group.append("path").attr("class", "trend-change-marker").attr("d", markerPath).style("fill", colors.trendChangeColor || "red").style("cursor", "help");
                group.on("mouseover", EventHandlers.trendChangeMouseOver).on("mouseout", EventHandlers.trendChangeMouseOut);
                group.transition().duration(dur).style("opacity", 1);
                return group;
             },
            update => update /* ... (same as before) ... */
                .on("mouseover", EventHandlers.trendChangeMouseOver).on("mouseout", EventHandlers.trendChangeMouseOut)
                .transition().duration(dur).style("opacity", 1)
                .attr("transform", d => {
                    const yValue = findYValue(d.date); const rotation = d.magnitude > 0 ? 180 : 0; const xPos = scales.x(d.date); const yPos = (yValue != null) ? scales.y(yValue) : null;
                    return (xPos != null && yPos != null && isFinite(xPos) && isFinite(yPos)) ? `translate(${xPos}, ${yPos}) rotate(${rotation})` : `translate(-1000, -1000)`;
                }),
            exit => exit /* ... (same as before) ... */
                .transition().duration(dur / 2).style("opacity", 0).remove()
        );
    },
     ...
     * @param {object} [options={}] - Rendering options.
     */
    updateGoalVisuals(goal, goalAchievedDate, focusWidth, focusHeight, options = {}) {
        const dur = options.isInteractive ? 0 : CONFIG.transitionDurationMs;
        ...
        if (ui.goalZoneRect && !ui.goalZoneRect.empty()) {
            ...
            if (goalWeight != null && isFinite(goalWeight)) {
                ...
                if (isFinite(yUpper) && isFinite(yLower) && yLower >= yUpper) {
                    ui.goalZoneRect.transition().duration(dur)
                        .attr("x", 0).attr("y", yUpper)
                        .attr("width", focusWidth).attr("height", Math.max(0, yLower - yUpper))
                        // Style handled by CSS var, ensure fill is set
                        .style("fill", colors.goal || "#9b59b6").style("fill-opacity", 0.06);
                } else {
        ...
        const showMarker = (goalAchievedDate instanceof Date && !isNaN(goalAchievedDate) && goal?.weight != null && isFinite(goal.weight));
        const markerData = showMarker ? [goalAchievedDate] : [];
        const markerSymbol = "🚩";

        if (ui.goalAchievedGroup && !ui.goalAchievedGroup.empty()) {
            const markers = ui.goalAchievedGroup.selectAll(".goal-achieved-marker").data(markerData, d => d.getTime());
            markers.join(
                enter => enter.append("text")
                    .attr("class", "goal-achieved-marker")
                    .attr("x", d => scales.x(d))
                    .attr("y", d => scales.y(goal.weight))
                    .attr("dy", "-0.5em").attr("text-anchor", "middle")
                    .style("font-size", "1.3em").style("fill", colors.goal || "#9b59b6")
                    .style("opacity", 0).style("cursor", "default").text(markerSymbol)
    ...
    updateRegressionBrushDisplay(interactiveRegressionRange, focusWidth) {
        if (!ui.regressionBrushGroup || !brushes.regression || !scales.x || !focusWidth) return;
        ...
        if (start instanceof Date && end instanceof Date && !isNaN(start.getTime()) && !isNaN(end.getTime())) {
            ...
            if (isFinite(pixelStart) && isFinite(pixelEnd) && pixelEnd > pixelStart &&
                pixelStart <= focusWidth && pixelEnd >= 0) // Allow partial visibility
            {
                ...
                if (clampedEnd > clampedStart) { brushSelection = [clampedStart, clampedEnd]; }
            }
        }

        // Show or hide brush elements based on whether we have a valid selection
        ui.regressionBrushGroup.selectAll(".overlay, .selection, .handle")
        ...
        const selectionChanged = (!currentSelection && brushSelection) ||
        ...
        if (selectionChanged) {
            ...
            ui.regressionBrushGroup.on("end.handler", EventHandlers.regressionBrushed); // Re-attach listener
        }
    },
...
export const ContextChartUpdater = {
    updateAxes() { if (!axes.xAxisContext || !scales.xContext) return; axes.xAxisContext.scale(scales.xContext); ui.contextXAxisGroup?.call(axes.xAxisContext); },
    updateChart(processedData) {
        if (!ui.contextArea || !ui.contextLine || !scales.xContext || !scales.yContext) return;
        const yContextScale = scales.yContext;
        const contextValueAccessor = d => d.sma ?? d.value;
        const contextAreaGen = d3.area().curve(d3.curveMonotoneX).x(d => scales.xContext(d.date)).y0(yContextScale.range()[0]).y1(d => yContextScale(contextValueAccessor(d)))
            .defined(d => d.date instanceof Date && !isNaN(d.date) && contextValueAccessor(d) != null && isFinite(scales.xContext(d.date)) && isFinite(yContextScale(contextValueAccessor(d))));
        const contextLineGen = d3.line().curve(d3.curveMonotoneX).x(d => scales.xContext(d.date)).y(d => yContextScale(contextValueAccessor(d)))
            .defined(d => d.date instanceof Date && !isNaN(d.date) && contextValueAccessor(d) != null && isFinite(scales.xContext(d.date)) && isFinite(yContextScale(contextValueAccessor(d))));
        ui.contextArea?.datum(processedData).attr("d", contextAreaGen).style("fill", colors.band || CONFIG.fallbackColors.band);
        ui.contextLine?.datum(processedData).attr("d", contextLineGen).style("stroke", colors.sma || CONFIG.fallbackColors.sma);
    }
};
export const BalanceChartUpdater = {
  updateAxes(balanceWidth) { /* ... (no changes needed here) ... */
      if (!balanceWidth || !axes.xBalanceAxis || !axes.yBalanceAxis || !scales.xBalance || !scales.yBalance) return;
      const dur = CONFIG.transitionDurationMs;
      axes.yBalanceAxis.scale(scales.yBalance); ui.balanceYAxisGroup?.transition().duration(dur).call(axes.yBalanceAxis); ui.balanceYAxisGroup?.select(".domain").remove();
      axes.xBalanceAxis.scale(scales.xBalance); ui.balanceXAxisGroup?.transition().duration(dur).call(axes.xBalanceAxis);
   },
  updateChart(visibleData, balanceWidth, options = {}) {
      const dur = options.isInteractive ? 0 : CONFIG.transitionDurationMs;
      if (!ui.balanceChartArea || !scales.xBalance || !scales.yBalance || !balanceWidth) return;
      const yZero = scales.yBalance(0); // Position of the zero line remains correct
      if (isNaN(yZero)) { console.error("BalanceChartUpdater: Invalid Y=0 position."); return; }

      ui.balanceZeroLine?.transition().duration(dur).attr("y1", yZero).attr("y2", yZero);

      const validBarData = visibleData.filter(d => d.netBalance != null && !isNaN(d.netBalance) && d.date instanceof Date && !isNaN(d.date) && isFinite(scales.xBalance(d.date)) && isFinite(scales.yBalance(d.netBalance)));
      const barWidth = Math.max(1, balanceWidth / Math.max(1, validBarData.length + 2)); // Add padding

      const bars = ui.balanceChartArea.selectAll(".balance-bar")
      ...
      bars.join(
          enter => enter.append("rect")
              ...
              .attr("y", yZero)
              .attr("height", 0)
              .style("fill", d => d.netBalance >= 0 ? (colors.surplus || CONFIG.fallbackColors.surplus) : (colors.deficit || CONFIG.fallbackColors.deficit))
              .on("mouseover", EventHandlers.balanceMouseOver)
              .on("mouseout", EventHandlers.balanceMouseOut)
              .call(enter => enter.transition().duration(dur)
                  // --- ADJUST Y POSITION ---
                  ...
                  .attr("y", d => d.netBalance >= 0 ? scales.yBalance(d.netBalance) : yZero )
                  // --- HEIGHT REMAINS ABSOLUTE DIFFERENCE ---
                  .attr("height", d => Math.abs(scales.yBalance(d.netBalance) - yZero))
              ),
          update => update
              ...
              .on("mouseover", EventHandlers.balanceMouseOver)
              .on("mouseout", EventHandlers.balanceMouseOut)
              .call(update => update.transition().duration(dur)
                  .attr("x", d => scales.xBalance(d.date) - barWidth / 2)
                  .attr("width", barWidth)
          ...
          exit => exit.call(exit => exit.transition().duration(dur / 2)
...
export const RateChartUpdater = {
    updateAxes(rateWidth) { /* ... (unchanged, axes updated in master) ... */
        if (!rateWidth || !axes.xRateAxis || !axes.yRateAxis || !scales.xRate || !scales.yRate) return;
        const dur = CONFIG.transitionDurationMs;
        axes.yRateAxis.scale(scales.yRate); ui.rateYAxisGroup?.transition().duration(dur).call(axes.yRateAxis); ui.rateYAxisGroup?.select(".domain").remove();
        axes.xRateAxis.scale(scales.xRate); ui.rateXAxisGroup?.transition().duration(dur).call(axes.xRateAxis);
    },
    updateChart(visibleData, rateWidth, options = {}) {
        const dur = options.isInteractive ? 0 : CONFIG.transitionDurationMs;
        if (!ui.rateChartArea || !scales.xRate || !scales.yRate || !rateWidth) return;
        ...
        if (ui.optimalGainZoneRect && !ui.optimalGainZoneRect.empty()) {
            const yUpper = yRateScale(upperBoundKgWk); const yLower = yRateScale(lowerBoundKgWk);
            if (!isNaN(yUpper) && !isNaN(yLower) && yLower >= yUpper) { ui.optimalGainZoneRect.transition().duration(dur).attr("y", yUpper).attr("height", yLower - yUpper).attr("width", rateWidth).style("display", null).style("fill", colors.optimalGainZone || CONFIG.fallbackColors.optimalGainZone); } else { ui.optimalGainZoneRect.style("display", "none"); }
        }
        const yZero = yRateScale(0); if (isNaN(yZero)) { console.error("RateChartUpdater: Invalid Y=0 position."); return; } ui.rateZeroLine?.transition().duration(dur).attr("y1", yZero).attr("y2", yZero);
        const rateLineGen = d3.line().x(d => scales.xRate(d.date)).y(d => scales.yRate(d.smoothedWeeklyRate)).defined(d => d.date instanceof Date && !isNaN(d.date) && d.smoothedWeeklyRate != null && isFinite(scales.xRate(d.date)) && isFinite(scales.yRate(d.smoothedWeeklyRate))); ui.rateLine?.datum(visibleData).transition().duration(dur).attr("d", rateLineGen).style("stroke", colors.rateLineColor || CONFIG.fallbackColors.rateLineColor);
        const rateMALineGen = d3.line().x(d => scales.xRate(d.date)).y(d => scales.yRate(d.rateMovingAverage)).defined(d => d.date instanceof Date && !isNaN(d.date) && d.rateMovingAverage != null && isFinite(scales.xRate(d.date)) && isFinite(scales.yRate(d.rateMovingAverage)));
        // Rate MA line visibility handled by MasterUpdater
        ui.rateMALine?.datum(visibleData).transition().duration(dur).attr("d", rateMALineGen).style("stroke", colors.rateMALine || CONFIG.fallbackColors.rateMALine);
    },
    addHoverDots(visibleData) { /* ... (unchanged - attaches handlers) ... */
         if (!ui.rateChartArea || !scales.xRate || !scales.yRate) return; ui.rateChartArea.selectAll(".rate-hover-dot").remove();
         const validData = visibleData.filter(d => d.smoothedWeeklyRate != null && isFinite(scales.xRate(d.date)) && isFinite(scales.yRate(d.smoothedWeeklyRate)));
         ui.rateChartArea.selectAll(".rate-hover-dot").data(validData).enter().append("circle").attr("class", "rate-hover-dot").attr("cx", d => scales.xRate(d.date)).attr("cy", d => scales.yRate(d.smoothedWeeklyRate)).attr("r", 8).style("fill", "transparent").style("cursor", "help")
         .on("mouseover", (event, d) => { let tt = `<strong>${Utils.formatDateShort(d.date)}</strong>`; tt += `<div>Smoothed Rate: <b>${Utils.formatValue(d.smoothedWeeklyRate, 2)}</b> kg/wk</div>`; if (d.rateMovingAverage != null) tt += `<div>Moving Avg: <b>${Utils.formatValue(d.rateMovingAverage, 2)}</b> kg/wk</div>`; EventHandlers._showTooltip(tt, event); })
         .on("mouseout", (event, d) => EventHandlers._hideTooltip());
    }
};
export const TDEEDiffChartUpdater = {
    updateAxes(tdeeDiffWidth) { /* ... (unchanged, axes updated in master) ... */
        if (!tdeeDiffWidth || !axes.xTdeeDiffAxis || !axes.yTdeeDiffAxis || !scales.xTdeeDiff || !scales.yTdeeDiff) return;
        const dur = CONFIG.transitionDurationMs;
        axes.yTdeeDiffAxis.scale(scales.yTdeeDiff); ui.tdeeDiffYAxisGroup?.transition().duration(dur).call(axes.yTdeeDiffAxis); ui.tdeeDiffYAxisGroup?.select(".domain").remove();
        axes.xTdeeDiffAxis.scale(scales.xTdeeDiff); ui.tdeeDiffXAxisGroup?.transition().duration(dur).call(axes.xTdeeDiffAxis);
    },
    updateChart(visibleData, options = {}) {
        const dur = options.isInteractive ? 0 : CONFIG.transitionDurationMs;
        if (!ui.tdeeDiffChartArea || !scales.xTdeeDiff || !scales.yTdeeDiff) return;
        const yZero = scales.yTdeeDiff(0); if (isNaN(yZero)) { console.error("TDEEDiffChartUpdater: Invalid Y=0 position."); return; } ui.tdeeDiffZeroLine?.transition().duration(dur).attr("y1", yZero).attr("y2", yZero);
        const tdeeDiffLineGen = d3.line().x(d => scales.xTdeeDiff(d.date)).y(d => scales.yTdeeDiff(d.avgTdeeDifference)).defined(d => d.date instanceof Date && !isNaN(d.date) && d.avgTdeeDifference != null && isFinite(scales.xTdeeDiff(d.date)) && isFinite(scales.yTdeeDiff(d.avgTdeeDifference)));
        ui.tdeeDiffLine?.datum(visibleData).transition().duration(dur).attr("d", tdeeDiffLineGen).style("stroke", colors.tdeeDiffLineColor || CONFIG.fallbackColors.tdeeDiffLineColor);
    },
    addHoverDots(visibleData) { /* ... (unchanged - attaches handlers) ... */
        if (!ui.tdeeDiffChartArea || !scales.xTdeeDiff || !scales.yTdeeDiff) return; ui.tdeeDiffChartArea.selectAll(".tdee-diff-hover-dot").remove();
        const validData = visibleData.filter(d => d.avgTdeeDifference != null && isFinite(scales.xTdeeDiff(d.date)) && isFinite(scales.yTdeeDiff(d.avgTdeeDifference)));
        ui.tdeeDiffChartArea.selectAll(".tdee-diff-hover-dot").data(validData).enter().append("circle").attr("class", "tdee-diff-hover-dot").attr("cx", d => scales.xTdeeDiff(d.date)).attr("cy", d => scales.yTdeeDiff(d.avgTdeeDifference)).attr("r", 8).style("fill", "transparent").style("cursor", "help")
        .on("mouseover", (event, d) => { const tt = `<strong>${Utils.formatDateShort(d.date)}</strong><br>TDEE Diff: <b>${Utils.formatValue(d.avgTdeeDifference, 0)}</b> kcal`; EventHandlers._showTooltip(tt, event); })
        .on("mouseout", (event, d) => EventHandlers._hideTooltip());
     }
};
export const ScatterPlotUpdater = {
    updateAxes() { /* ... (unchanged, axes updated in master) ... */
        if (!axes.xScatterAxis || !axes.yScatterAxis || !scales.xScatter || !scales.yScatter) return;
        const dur = CONFIG.transitionDurationMs;
        axes.xScatterAxis.scale(scales.xScatter); ui.correlationScatterXAxisGroup?.transition().duration(dur).call(axes.xScatterAxis);
        axes.yScatterAxis.scale(scales.yScatter); ui.correlationScatterYAxisGroup?.transition().duration(dur).call(axes.yScatterAxis);
    },
    updateChart(scatterData, options = {}) {
        const dur = options.isInteractive ? 0 : CONFIG.transitionDurationMs;
        if (!ui.scatterDotsGroup || !scales.xScatter || !scales.yScatter) return;
        const validScatterData = (Array.isArray(scatterData) ? scatterData : []).filter(d => d.avgNetCal != null && !isNaN(d.avgNetCal) && d.weeklyRate != null && !isNaN(d.weeklyRate) && isFinite(scales.xScatter(d.avgNetCal)) && isFinite(scales.yScatter(d.weeklyRate)));
        const dots = ui.scatterDotsGroup.selectAll(".scatter-dot").data(validScatterData, d => d.weekKey);
        dots.join(
            enter => enter.append("circle").attr("class", "scatter-dot").attr("cx", d => scales.xScatter(d.avgNetCal)).attr("cy", d => scales.yScatter(d.weeklyRate)).attr("r", 4).style("fill", colors.scatterDotColor || CONFIG.fallbackColors.scatterDotColor).style("opacity", 0).style("cursor", "help").on("mouseover", EventHandlers.scatterMouseOver).on("mouseout", EventHandlers.scatterMouseOut).call(enter => enter.transition().duration(dur).style("opacity", 0.7)),
            update => update.on("mouseover", EventHandlers.scatterMouseOver).on("mouseout", EventHandlers.scatterMouseOut).transition().duration(dur).attr("cx", d => scales.xScatter(d.avgNetCal)).attr("cy", d => scales.yScatter(d.weeklyRate)).style("opacity", 0.7),
            exit => exit.transition().duration(dur / 2).style("opacity", 0).remove()
        );
    },
};