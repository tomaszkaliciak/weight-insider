...
export const WeeklySummaryUpdater = {
  ...
  _renderTable({ weeklyData, sortKey, sortDir }) {
    ...
    if (!Array.isArray(weeklyData) || weeklyData.length === 0) {
      ...
      emptyMsg.style("display", null).text("No weekly data available for the selected analysis range.");
      return;
    }
    ...
    const columnsConfig = [
      { key: "weekStartDate", label: "Week Start", numeric: false, sortable: true },
      { key: "avgWeight", label: "Avg Wgt (kg)", numeric: true, sortable: true },
      { key: "weeklyRate", label: "Rate (kg/wk)", numeric: true, sortable: true },
      { key: "avgIntake", label: "Avg Intake", numeric: true, sortable: true },
      { key: "avgExpenditure", label: "Avg GFit", numeric: true, sortable: true },
    ...
    thead.selectAll("tr").data([null]).join("tr")
      .selectAll("th").data(columnsConfig, d => d.key).join("th")
        .attr("class", d => `${d.numeric ? "numeric" : ""} ${d.sortable ? "sortable" : ""}`.trim())
        .classed("sorted-asc", d => d.sortable && d.key === sortKey && sortDir === "asc")
        .classed("sorted-desc", d => d.sortable && d.key === sortKey && sortDir === "desc")
        .text(d => d.label)
        .on("click", (event, d) => {
          if (!d.sortable) return;
          let newDirection = 'asc';
          if (d.key === sortKey) { newDirection = sortDir === 'asc' ? 'desc' : 'asc'; }
          // Dispatch action to update sort options in state
          StateManager.dispatch({ type: 'SET_SORT_OPTIONS', payload: { columnKey: d.key, direction: newDirection } });
        });

    const sortedData = this._sortData(weeklyData, sortKey, sortDir); // Sort the data passed in

    const fv = Utils.formatValue; const fd = Utils.formatDateShort;
    const rowColumns = [
    ...
    rows.join(
      ...
      update => {
        update.selectAll("td").data(d => rowColumns.map(col => ({ value: d[col.key], format: col.format, numeric: col.numeric })))
  ...
  init() {
    ...
    StateManager.subscribe((stateChanges) => {
        ...
        if (stateChanges.action.type.includes('WEEKLY_SUMMARY') || stateChanges.action.type.includes('SORT_OPTIONS')) {
            ...
            const { columnKey: sortKey, direction: sortDir } = Selectors.selectSortOptions(newState);
            this._renderTable({ weeklyData, sortKey, sortDir });
        }
    });
    console.log("[WeeklySummaryUpdater Init] Subscribed to relevant state changes.");
    ...
    const { columnKey: initialSortKey, direction: initialSortDir } = Selectors.selectSortOptions(initialState);
    this._renderTable({ weeklyData: initialWeeklyData, sortKey: initialSortKey, sortDir: initialSortDir });
  },
};