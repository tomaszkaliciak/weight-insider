...
export const selectInteractiveRegressionRange = (state) => state.interactiveRegressionRange;
...
export const selectCorrelationScatterData = (state) => state.correlationScatterData;
...
export const selectIsGoalAchieved = (state) => {
    if (state.goal.weight == null || !state.processedData || state.processedData.length === 0) {
        return false;
    }
    // Use the goalAchievedDate from state if it's already set
    if (state.goalAchievedDate instanceof Date && !isNaN(state.goalAchievedDate)) {
        return true;
    }
    // Fallback check based on latest SMA/weight (less reliable than explicit date)
    const lastSmaEntry = [...state.processedData].reverse().find(d => d.sma != null);
    const lastWeightEntry = [...state.processedData].reverse().find(d => d.value != null);
...
export const selectFocusChartMarkerData = (state) => {
    ...
    return {
        visibleRawWeightData: state.filteredData?.filter((d) => d.value != null) || [],
