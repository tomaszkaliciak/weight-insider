...
// No direct StateManager or UI cache imports needed here.
...
export const DataService = {
    ...
    async fetchData() {
        ...
        try {
            ...
            Utils.showStatusMessage(`Error loading data: ${error.message}. Chart may be incomplete or empty.`, "error", 10000);
            return { weights: {}, calorieIntake: {}, googleFitExpenditure: {}, bodyFat: {} }; // Return empty structure on error
        }
    },
    ...
    mergeRawData(rawDataObjects) {
        ...
        const allDates = new Set([
            ...Object.keys(weights), ...Object.keys(calorieIntake),
            ...Object.keys(googleFitExpenditure), ...Object.keys(bodyFat)
        ]);
        let mergedData = [];
        for (const dateStr of allDates) {
            // Basic validation of date string format (YYYY-MM-DD)
            if (!/^\d{4}-\d{2}-\d{2}$/.test(dateStr)) {
                 console.warn(`DataService: Skipping invalid date key format: ${dateStr}`);
                 continue;
            }
            const dateObj = new Date(dateStr + 'T00:00:00'); // Ensure parsing as local date at midnight
            if (isNaN(dateObj.getTime())) {
                console.warn(`DataService: Skipping invalid date string after parsing: ${dateStr}`);
                continue;
            }
            ...
            mergedData.push({
                ...
                sma: null, ema: null, stdDev: null, lowerBound: null, upperBound: null,
                lbm: null, fm: null, lbmSma: null, fmSma: null, isOutlier: false,
        ...
        console.log(`DataService: Merged data for ${mergedData.length} unique dates.`);
        return mergedData;
    },
    ...
    processData(rawData) {
        ...
        processed = this._calculateRollingVolatility(processed, CONFIG.ROLLING_VOLATILITY_WINDOW);
    ...
    _calculateBodyComposition(data) {
        return data.map(d => {
            let lbm = null; let fm = null;
            if (d.value != null && d.bfPercent != null && !isNaN(d.value) && !isNaN(d.bfPercent) && d.bfPercent >= 0 && d.bfPercent < 100) {
    ...
    _calculateSMAAndStdDev(data) {
        ...
        return data.map((d, i, arr) => {
            const windowDataPoints = arr.slice(Math.max(0, i - windowSize + 1), i + 1);
            const validValuesInWindow = windowDataPoints.map(p => p.value).filter(v => v != null && !isNaN(v));
            let sma = null, stdDev = null, lowerBound = null, upperBound = null;
            if (validValuesInWindow.length > 0) {
                sma = d3.mean(validValuesInWindow);
                stdDev = (validValuesInWindow.length > 1 && typeof ss?.standardDeviation === 'function') ? ss.standardDeviation(validValuesInWindow) : 0;
                lowerBound = (sma != null && stdDev != null) ? sma - stdDevMult * stdDev : null;
                upperBound = (sma != null && stdDev != null) ? sma + stdDevMult * stdDev : null;
            }
            const validLbmInWindow = windowDataPoints.map(p => p.lbm).filter(v => v != null && !isNaN(v));
            const lbmSma = validLbmInWindow.length > 0 ? d3.mean(validLbmInWindow) : null;
            const validFmInWindow = windowDataPoints.map(p => p.fm).filter(v => v != null && !isNaN(v));
            const fmSma = validFmInWindow.length > 0 ? d3.mean(validFmInWindow) : null;
    ...
    _calculateEMA(data) {
        ...
        return data.map((d, i) => {
            let currentEMA = null; const currentValue = d.value;
            if (currentValue != null && !isNaN(currentValue)) {
                if (previousEMA === null) { currentEMA = currentValue; }
                else { currentEMA = currentValue * alpha + previousEMA * (1 - alpha); }
                previousEMA = currentEMA;
            } else if (previousEMA !== null) { currentEMA = previousEMA; } // Carry forward last valid EMA if current value is null
    ...
    _identifyOutliers(data) {
        ...
        return data.map(d => {
            let isOutlier = false;
            if (d.value != null && d.sma != null && d.stdDev != null && d.stdDev > 0.01) { // Check stdDev > 0 to avoid division issues
                if (Math.abs(d.value - d.sma) > threshold * d.stdDev) { isOutlier = true; }
            }
            return { ...d, isOutlier };
        });
    },
    _calculateRollingVolatility(data, windowSize) {
        ...
        return data.map((d, i, arr) => {
            ...
            const validDeviations = windowDataPoints
                .filter(p => p.value != null && p.sma != null && !p.isOutlier && !isNaN(p.value) && !isNaN(p.sma))
                .map(p => p.value - p.sma);
            if (validDeviations.length >= 2 && typeof ss?.standardDeviation === 'function') {
                try { rollingVolatility = ss.standardDeviation(validDeviations); }
                catch (e) { console.warn(`DataService: Error calculating stdev for rolling volatility at index ${i}`, e); rollingVolatility = null; }
            }
    ...
    _calculateDailyRatesAndTDEETrend(data) {
        return data.map((d, i, arr) => {
            let dailySmaRate = null; let tdeeTrend = null;
            if (i > 0) {
                const prev = arr[i - 1];
                if (prev.sma != null && d.sma != null && d.date instanceof Date && prev.date instanceof Date && !isNaN(d.date) && !isNaN(prev.date)) {
                    const timeDiffDays = (d.date.getTime() - prev.date.getTime()) / 86400000;
                    ...
                    if (timeDiffDays > 0 && timeDiffDays <= CONFIG.movingAverageWindow) {
                        ...
                        if (prev.calorieIntake != null && !isNaN(prev.calorieIntake)) { // Use previous day's intake for trend TDEE
                            const dailyDeficitSurplusKcals = dailySmaRate * CONFIG.KCALS_PER_KG;
    ...
    _calculateAdaptiveTDEE(data) {
        ...
        return data.map((d, i, arr) => {
            ...
            if (i >= windowSize - 1) {
                ...
                const validIntakes = windowData.map(p => p.calorieIntake).filter(v => v != null && !isNaN(v));
                // Ensure start/end points have valid SMA and dates
                if (validIntakes.length >= windowSize * minDataRatio &&
                    startPoint.sma != null && endPoint.sma != null &&
                    startPoint.date instanceof Date && !isNaN(startPoint.date) &&
                    endPoint.date instanceof Date && !isNaN(endPoint.date))
                {
                    const avgIntakeWindow = d3.mean(validIntakes);
                    const totalSmaChange = endPoint.sma - startPoint.sma;
                    const actualDaysInWindow = (endPoint.date.getTime() - startPoint.date.getTime()) / 86400000;
                    if (actualDaysInWindow > 0) { // Avoid division by zero if dates are same
                        const avgDailySmaChange = totalSmaChange / actualDaysInWindow;
                        const avgDailyDeficitSurplusKcals = avgDailySmaChange * CONFIG.KCALS_PER_KG;
    ...
    _smoothRatesAndTDEEDifference(data) {
        ...
        const smoothedDailyRates = Utils.calculateRollingAverage(dailyRates, CONFIG.rateOfChangeSmoothingWindow); // Use CONFIG
        const tdeeDifferences = data.map(d => (d.tdeeTrend != null && d.googleFitTDEE != null && !isNaN(d.tdeeTrend) && !isNaN(d.googleFitTDEE)) ? d.tdeeTrend - d.googleFitTDEE : null);
        const smoothedTdeeDifferences = Utils.calculateRollingAverage(tdeeDifferences, CONFIG.tdeeDiffSmoothingWindow); // Use CONFIG
        return data.map((d, i) => ({
            ...d,
            smoothedWeeklyRate: smoothedDailyRates[i] != null ? smoothedDailyRates[i] * 7 : null,
    ...
    _calculateRateMovingAverage(data) {
        ...
        const rateMovingAverage = Utils.calculateRollingAverage(weeklyRates, CONFIG.rateMovingAverageWindow); // Use CONFIG
        return data.map((d, i) => ({ ...d, rateMovingAverage: rateMovingAverage[i] }));
    },

    // --- Calculation Helpers ---
    _calculateDailyBalance(intake, expenditure) {
        return (intake != null && expenditure != null && !isNaN(intake) && !isNaN(expenditure)) ? intake - expenditure : null;
    },

    // --- Regression & Trend Calculations (Used by StatsManager) ---
    calculateLinearRegression(dataPoints, startDate) {
        const validData = dataPoints.filter(d => d.value != null && !d.isOutlier && d.date instanceof Date && !isNaN(d.date));
        const filteredData = (startDate instanceof Date && !isNaN(startDate)) ? validData.filter(d => d.date >= startDate) : validData;
        ...
        const dataForRegression = filteredData.map(d => [(d.date.getTime() - firstDateMs) / dayInMillis, d.value]);

        try {
            if (!ss || typeof ss.linearRegression !== 'function') { throw new Error("simple-statistics linearRegression not available."); }
    ...
    calculateTrendWeight(startDate, initialWeight, weeklyIncrease, targetDate) {
        if (!(startDate instanceof Date) || isNaN(startDate) || initialWeight == null || isNaN(initialWeight) || weeklyIncrease == null || isNaN(weeklyIncrease) || !(targetDate instanceof Date) || isNaN(targetDate)) {
            return null;
        }
        const msPerWeek = 7 * 86400000;
        const weeksElapsed = (targetDate.getTime() - startDate.getTime()) / msPerWeek;
    ...
    calculateWeeklyStats(processedData, startDate, endDate) {
        const rangeData = (startDate && endDate) ?
            processedData.filter(d => d.date instanceof Date && d.date >= startDate && d.date <= endDate) :
            processedData;
        ...
        groupedByWeek.forEach((weekData, weekKey) => {
            ...
            if (weekData.length > 0) { // Ensure week has data from range
                const avgMetric = (data, metric) => {
                    const valid = data.map(d => d[metric]).filter(v => v != null && !isNaN(v));
