...
function reducer(currentState, action) {
    ...
    switch (action.type) {
        ...
        case 'DELETE_ANNOTATION':
            nextState.annotations = nextState.annotations.filter(a => a.id !== action.payload.id);
            break;
        case 'SET_PLATEAUS':
        ...
        case 'SET_GOAL_ACHIEVED_DATE':
            nextState.goalAchievedDate = action.payload instanceof Date || action.payload === null ? action.payload : null;
            break;
        case 'SET_ANALYSIS_RANGE':
            nextState.analysisRange = { ...nextState.analysisRange, ...(action.payload || {}) };
            if (nextState.analysisRange.start && !(nextState.analysisRange.start instanceof Date)) {
                 const parsedDate = new Date(nextState.analysisRange.start);
                 nextState.analysisRange.start = !isNaN(parsedDate) ? parsedDate : null;
            }
             if (nextState.analysisRange.end && !(nextState.analysisRange.end instanceof Date)) {
        ...
        case 'SET_INTERACTIVE_REGRESSION_RANGE':
            nextState.interactiveRegressionRange = { ...nextState.interactiveRegressionRange, ...(action.payload || {}) };
            if (nextState.interactiveRegressionRange.start && !(nextState.interactiveRegressionRange.start instanceof Date)) {
                 const parsedDate = new Date(nextState.interactiveRegressionRange.start);
                 nextState.interactiveRegressionRange.start = !isNaN(parsedDate) ? parsedDate : null;
            }
             if (nextState.interactiveRegressionRange.end && !(nextState.interactiveRegressionRange.end instanceof Date)) {
                 const parsedDate = new Date(nextState.interactiveRegressionRange.end);
                 nextState.interactiveRegressionRange.end = !isNaN(parsedDate) ? parsedDate : null;
            }
            break;
        case 'SET_THEME':
        ...
        case 'TOGGLE_SERIES_VISIBILITY':
            if (nextState.seriesVisibility.hasOwnProperty(action.payload.seriesId)) {
                nextState.seriesVisibility[action.payload.seriesId] = !!action.payload.isVisible;
            }
            break;
        case 'SET_HIGHLIGHTED_DATE':
            nextState.highlightedDate = action.payload instanceof Date || action.payload === null ? action.payload : null;
            break;
        ...
        case 'SET_LAST_ZOOM_TRANSFORM':
            if (action.payload && typeof action.payload.k === 'number' && typeof action.payload.x === 'number' && typeof action.payload.y === 'number') {
                nextState.lastZoomTransform = { k: action.payload.k, x: action.payload.x, y: action.payload.y };
            } else {
        ...
        case 'SET_SORT_OPTIONS':
            nextState.sortColumnKey = action.payload.columnKey ?? nextState.sortColumnKey;
            nextState.sortDirection = action.payload.direction ?? nextState.sortDirection;
            break;
        case 'LOAD_SETTINGS':
            nextState.settings = { ...nextState.settings, ...(action.payload || {}) };
            break;
        case 'UPDATE_TREND_CONFIG':
            const { startDate, initialWeight, weeklyIncrease1, weeklyIncrease2 } = action.payload || {};
            const parsedStartDate = startDate instanceof Date && !isNaN(startDate.getTime()) ? startDate : null;
            const parsedInitialWeight = initialWeight != null && isFinite(initialWeight) ? parseFloat(initialWeight) : null;
            const parsedWeeklyIncrease1 = weeklyIncrease1 != null && isFinite(weeklyIncrease1) ? parseFloat(weeklyIncrease1) : null;
            const parsedWeeklyIncrease2 = weeklyIncrease2 != null && isFinite(weeklyIncrease2) ? parseFloat(weeklyIncrease2) : null;
            const isValid = parsedStartDate && parsedInitialWeight !== null && parsedWeeklyIncrease1 !== null && parsedWeeklyIncrease2 !== null;
            nextState.trendConfig = { startDate: parsedStartDate, initialWeight: parsedInitialWeight, weeklyIncrease1: parsedWeeklyIncrease1, weeklyIncrease2: parsedWeeklyIncrease2, isValid: isValid };
            break;
        case 'SET_DISPLAY_STATS': // Store display stats
            nextState.displayStats = typeof action.payload === 'object' && action.payload !== null ? action.payload : {};
            break;
...
function _notifyGeneralSubscribers(newState, previousState, action) {
    ...
    generalSubscribers.forEach(listener => {
        try {
            ...
            console.error("[StateManager] Error in general subscriber:", error, { listener });
        }
    });
}
    ...
    if (specificSubscribers[eventName]) {
        ...
        specificSubscribers[eventName].forEach(listener => {
            try {
                ...
                console.error(`[StateManager] Error in specific subscriber for ${eventName}:`, error, { listener });
            }
        });
    }
}
...
export const StateManager = {
    ...
    dispatch(action) {
        ...
        const specificEventsMap = {
            ...
            'SET_INTERACTIVE_REGRESSION_RANGE': 'state:interactiveRegressionRangeChanged',
        ...
        if (eventName) {
            ...
            let eventPayload;
            switch (action.type) {
                case 'LOAD_ANNOTATIONS': case 'ADD_ANNOTATION': case 'DELETE_ANNOTATION':
                    eventPayload = { annotations: newStateForEvent.annotations }; break;
                case 'TOGGLE_SERIES_VISIBILITY':
                    eventPayload = { visibility: newStateForEvent.seriesVisibility }; break;
                case 'LOAD_GOAL': case 'SET_GOAL_ACHIEVED_DATE':
                    eventPayload = { goal: newStateForEvent.goal, achievedDate: newStateForEvent.goalAchievedDate }; break;
                case 'SET_THEME':
                    eventPayload = { theme: newStateForEvent.currentTheme }; break;
                case 'SET_ANALYSIS_RANGE':
                    eventPayload = { range: newStateForEvent.analysisRange }; break;
                case 'SET_INTERACTIVE_REGRESSION_RANGE':
                    eventPayload = { range: newStateForEvent.interactiveRegressionRange }; break;
                case 'SET_FILTERED_DATA':
                    eventPayload = { data: newStateForEvent.filteredData }; break;
                case 'SET_WEEKLY_SUMMARY':
                    eventPayload = { data: newStateForEvent.weeklySummaryData }; break;
                case 'SET_CORRELATION_DATA':
                    eventPayload = { data: newStateForEvent.correlationScatterData }; break;
                case 'SET_PLATEAUS':
                    eventPayload = { plateaus: newStateForEvent.plateaus }; break;
                case 'SET_TREND_CHANGES':
                    eventPayload = { trendChangePoints: newStateForEvent.trendChangePoints }; break;
                case 'SET_REGRESSION_RESULT':
                    eventPayload = { result: newStateForEvent.regressionResult }; break;
                case 'UPDATE_TREND_CONFIG':
                    eventPayload = { config: newStateForEvent.trendConfig }; break;
                case 'SET_SORT_OPTIONS':
                    eventPayload = { columnKey: newStateForEvent.sortColumnKey, direction: newStateForEvent.sortDirection }; break;
                case 'SET_DISPLAY_STATS':
                    eventPayload = newStateForEvent.displayStats; break; // Pass the stats object
                case 'INITIALIZATION_COMPLETE':
                    eventPayload = {}; break; // No specific payload needed
                 case 'SET_HIGHLIGHTED_DATE':
                    eventPayload = { date: newStateForEvent.highlightedDate }; break;
                 case 'SET_PINNED_TOOLTIP':
                     eventPayload = { data: newStateForEvent.pinnedTooltipData }; break;
                  case 'SET_ACTIVE_HOVER_DATA':
                      eventPayload = { data: newStateForEvent.activeHoverData }; break;
                default:
                    // Fallback for events not explicitly listed above
                    console.warn(`[StateManager] Missing specific event payload definition for ${action.type} in dispatch.`);
    ...
    subscribeToSpecificEvent(eventName, listener) {
        ...
        return () => {
            if (specificSubscribers[eventName]) {
                ...
                console.log(`[StateManager] Specific subscriber unsubscribed from: ${eventName}`);
                // Optional: Clean up Set if empty
                // if (specificSubscribers[eventName].size === 0) {
                //     delete specificSubscribers[eventName];
                // }
            }
        };
    }
