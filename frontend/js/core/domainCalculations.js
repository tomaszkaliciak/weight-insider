 ...
 */
export function calculateContextYDomain(
  processedData,
  isSmaVisible,
  isBandVisible,
) {
  ...
  dataForExtent.forEach((d) => {
    ...
    if (baseValue != null && isFinite(baseValue)) {
      ...
      if (isBandVisible) {
        ...
        lower =
          d.lowerBound != null && isFinite(d.lowerBound)
            ? d.lowerBound
            : baseValue;
        upper =
          d.upperBound != null && isFinite(d.upperBound)
            ? d.upperBound
            : baseValue;
      }
  ...
  if (yMin === Infinity || yMax === -Infinity || dataForExtent.length === 0) {
    ...
    yMin -= 1;
    yMax += 1; // Handle single value case
  } else {
...
export function calculateFocusYDomain(
  ...
  filteredData.forEach((d) => {
    ...
    if (visibility.raw && d.value != null && !d.isOutlier)
      updateExtent(d.value); // Include non-outlier raw
  ...
  if (regressionResult) {
    if (visibility.regression && Array.isArray(regressionResult.points)) {
      ...
      const filteredDatesSet = new Set(
        filteredData.map((d) => d.date?.getTime()),
      );
      ...
      if (regressionResult.points.length > 0) {
        ...
        const lastRegPoint =
          regressionResult.points[regressionResult.points.length - 1];
        if (
          firstRegPoint.date &&
          filteredDatesSet.has(firstRegPoint.date.getTime())
        ) {
          updateExtent(firstRegPoint.regressionValue);
        }
        if (
          lastRegPoint.date &&
          filteredDatesSet.has(lastRegPoint.date.getTime())
        ) {
  ...
  if (visibility.goal && goal.weight != null) {
    ...
    if (
      lastSmaPoint?.date &&
      filteredDatesSet.has(lastSmaPoint.date.getTime())
    ) {
      updateExtent(lastSmaPoint.sma);
    }
    if (
      goal.date instanceof Date &&
      filteredDatesSet.has(goal.date.getTime())
    ) {
  ...
  if (yMin === Infinity || yMax === -Infinity) {
    ...
    console.warn(
      "DomainCalculations: No valid data for focus Y domain in current filtered view. Returning invalid domain.",
    );
    return [NaN, NaN];
  } else if (yMin === yMax) {
