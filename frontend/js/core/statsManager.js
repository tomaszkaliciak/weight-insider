...
const ss = window.ss || {
    sampleCorrelation: () => { console.warn("ss.sampleCorrelation unavailable"); return NaN; },
    linearRegression: () => { console.warn("ss.linearRegression unavailable"); return { m: NaN, b: NaN }; },
    standardDeviation: (arr) => {
        console.warn("ss.standardDeviation unavailable, using basic implementation.");
        const n = arr.length; if (n < 2) return 0;
        const meanVal = arr.reduce((a, b) => a + b, 0) / n;
        return Math.sqrt(arr.map((x) => Math.pow(x - meanVal, 2)).reduce((a, b) => a + b, 0) / (n - 1));
    },
    sum: (arr) => arr.reduce((a, b) => a + b, 0),
    mean: (arr) => arr.length === 0 ? 0 : arr.reduce((a, b) => a + b, 0) / arr.length,
    tDistributionQuantile: (p, df) => { console.warn("ss.tDistributionQuantile unavailable, using placeholder 1.96 for CI"); return 1.96; }
};
...
export const StatsManager = {
    ...
    _calculateCountInRange(data, field, startDate, endDate) { /* ... */
        ...
        if (!Array.isArray(data) || !startDate || !endDate || startDate > endDate) return defaultResult;
        const totalDays = Math.round((endDate.getTime() - startDate.getTime()) / 86400000) + 1;
        if (totalDays <= 0) return defaultResult;
        const rangeData = data.filter(d => d.date >= startDate && d.date <= endDate);
        const count = rangeData.filter(d => d[field] != null && !isNaN(d[field])).length;
    ...
    _calculateCurrentRate(allProcessedData, analysisEndDate) { /* ... */
        if (!Array.isArray(allProcessedData) || allProcessedData.length === 0 || !analysisEndDate) return null;
        let lastRate = null;
        for (let i = allProcessedData.length - 1; i >= 0; i--) {
            const d = allProcessedData[i];
            if (d.date instanceof Date && d.date <= analysisEndDate && d.smoothedWeeklyRate != null && !isNaN(d.smoothedWeeklyRate)) {
    ...
    _calculateVolatility(processedData, startDate, endDate) { /* ... */
        if (!Array.isArray(processedData) || !startDate || !endDate || startDate > endDate) return null;
        const viewData = processedData.filter(d => d.date >= startDate && d.date <= endDate);
        const deviations = viewData
            .filter(d => d.sma != null && d.value != null && !d.isOutlier)
            .map(d => d.value - d.sma);
        return deviations.length >= 2 && typeof ss?.standardDeviation === 'function' ? ss.standardDeviation(deviations) : null;
     },
    _calculateTDEEFromTrend(avgIntake, weeklyKgChange) { /* ... */
        if (avgIntake == null || weeklyKgChange == null || isNaN(avgIntake) || isNaN(weeklyKgChange)) return null;
    ...
    _calculateNetCalRateCorrelation(weeklyStats) { /* ... */
        ...
        if (!Array.isArray(weeklyStats) || weeklyStats.length < CONFIG.MIN_WEEKS_FOR_CORRELATION) return null;
        const validWeeklyStats = weeklyStats.filter(w => w.avgNetCal != null && !isNaN(w.avgNetCal) && w.weeklyRate != null && !isNaN(w.weeklyRate));
    ...
    _estimateTimeToGoal(currentWeight, goalWeight, weeklyChange, goalAchieved = false) { /* ... */
        if (goalAchieved) return "Goal Achieved!";
        if (currentWeight == null || goalWeight == null || weeklyChange == null || isNaN(currentWeight) || isNaN(goalWeight) || isNaN(weeklyChange)) return "N/A";
        const weightDifference = goalWeight - currentWeight;
        if (Math.abs(weeklyChange) < 0.01) return "Trend flat";
        if ((weeklyChange > 0 && weightDifference < 0) || (weeklyChange < 0 && weightDifference > 0)) return "Trending away";
        ...
        if (weeksNeeded < 8) return `~${Math.round(weeksNeeded)} week${weeksNeeded >= 1.5 ? 's' : ''}`;
        const monthsNeeded = weeksNeeded / (365.25 / 12 / 7);
        if (monthsNeeded < 18) return `~${Math.round(monthsNeeded)} month${monthsNeeded >= 1.5 ? 's' : ''}`;
        return `~${(monthsNeeded / 12).toFixed(1)} years`;
     },
    _calculateRequiredRateForGoal(currentWeight, goalWeight, goalDate) { /* ... */
        if (currentWeight == null || goalWeight == null || !(goalDate instanceof Date) || isNaN(goalDate) || isNaN(currentWeight) || isNaN(goalWeight)) return null;
        const today = new Date(); today.setHours(0, 0, 0, 0);
        const targetDate = new Date(goalDate); targetDate.setHours(0, 0, 0, 0);
     ...
     _detectPlateaus(processedData) { /* ... */
        ...
        for (let i = 0; i < processedData.length; i++) {
            ...
            const isFlat = rate != null && !isNaN(rate) && Math.abs(rate) < rateThreshold;
            if (isFlat && currentPlateauStart === null) {
                ...
                if (!(endDate instanceof Date) || isNaN(endDate.getTime()) || !(currentPlateauStartDateObj instanceof Date) || isNaN(currentPlateauStartDateObj.getTime())) {
                    currentPlateauStart = null; currentPlateauStartDateObj = null; continue;
                }
                const durationDays = (endDate.getTime() - currentPlateauStartDateObj.getTime()) / 86400000;
                if (durationDays >= minDurationDays - 1) {
                    plateaus.push({ startDate: currentPlateauStartDateObj, endDate: endDate });
                }
                currentPlateauStart = null; currentPlateauStartDateObj = null;
            }
        }
    ...
    _detectTrendChanges(processedData) { /* ... */
        if (!Array.isArray(processedData) || processedData.length < CONFIG.trendChangeWindowDays * 2) return [];
        ...
        const calculateSlope = (dataSegment) => { /* ... */
            const validPoints = dataSegment.filter(p => p.sma != null && p.date instanceof Date && !isNaN(p.date));
            if (validPoints.length < 2) return null;
            validPoints.sort((a, b) => a.date - b.date); // Ensure sorted by date
            const first = validPoints[0];
            const last = validPoints[validPoints.length - 1];
            const timeDiffDays = (last.date.getTime() - first.date.getTime()) / 86400000;
        ...
        for (let i = windowSize; i < processedData.length - windowSize; i++) { /* ... */
            ...
            if (!(currentDate instanceof Date) || isNaN(currentDate.getTime())) continue; // Skip invalid dates
            ...
            if (slopeBefore != null && slopeAfter != null && !isNaN(slopeBefore) && !isNaN(slopeAfter)) {
    ...
    _calculateDerivedData(stateSnapshot) {
        ...
        const effectiveRegRange = Selectors.selectEffectiveRegressionRange(stateSnapshot);

        // Filter Data
        if (processedData.length > 0 && analysisRange.start && analysisRange.end) {
            results.filteredData = processedData.filter(d => d.date instanceof Date && d.date >= analysisRange.start && d.date <= analysisRange.end);
        } else { results.filteredData = []; }

        // All-Time Stats
        const validWeightDataAll = rawData.filter(d => d.value != null && !isNaN(d.value));
        if (validWeightDataAll.length > 0) { /* ... */
            displayStats.startingWeight = validWeightDataAll[0].value;
            displayStats.currentWeight = validWeightDataAll[validWeightDataAll.length - 1].value;
            ...
            displayStats.totalChange = (displayStats.startingWeight != null && displayStats.currentWeight != null) ? displayStats.currentWeight - displayStats.startingWeight : null;
        } else { /* ... defaults ... */
             Object.assign(displayStats, { startingWeight: null, currentWeight: null, maxWeight: null, maxWeightDate: null, minWeight: null, minWeightDate: null, totalChange: null });
        }
        const lastSmaEntry = [...processedData].reverse().find(d => d.sma != null);
        displayStats.currentSma = lastSmaEntry?.sma ?? displayStats.currentWeight ?? null;

        // Body Comp Stats
        const validLbmSmaData = processedData.filter(d => d.lbmSma != null && !isNaN(d.lbmSma));
        const validFmSmaData = processedData.filter(d => d.fmSma != null && !isNaN(d.fmSma));
        displayStats.startingLbm = validLbmSmaData[0]?.lbmSma ?? null; /* ... etc ... */
        displayStats.currentLbmSma = validLbmSmaData[validLbmSmaData.length - 1]?.lbmSma ?? null;
        displayStats.totalLbmChange = (displayStats.startingLbm != null && displayStats.currentLbmSma != null) ? displayStats.currentLbmSma - displayStats.startingLbm : null;
        const startingFmSma = validFmSmaData[0]?.fmSma ?? null;
        displayStats.currentFmSma = validFmSmaData[validFmSmaData.length - 1]?.fmSma ?? null;
        displayStats.totalFmChange = (startingFmSma != null && displayStats.currentFmSma != null) ? displayStats.currentFmSma - startingFmSma : null;
        ...
        if (analysisRange.start && analysisRange.end && processedData.length > 0) { /* ... */
            ...
            if (results.filteredData.length > 0) { /* ... */
                 results.weeklySummaryData = DataService.calculateWeeklyStats(processedData, analysisRange.start, analysisRange.end);
                 results.correlationScatterData = results.weeklySummaryData.filter(w => w.avgNetCal != null && w.weeklyRate != null);
                 displayStats.netCalRateCorrelation = this._calculateNetCalRateCorrelation(results.weeklySummaryData);
                 displayStats.currentWeeklyRate = this._calculateCurrentRate(processedData, analysisRange.end);
                 displayStats.volatility = this._calculateVolatility(processedData, analysisRange.start, analysisRange.end);
                 const latestFilteredWithVol = [...results.filteredData].reverse().find(d => d.rollingVolatility != null);
                 displayStats.rollingVolatility = latestFilteredWithVol?.rollingVolatility ?? null;
                 displayStats.avgIntake = this._calculateAverageInRange(processedData, 'calorieIntake', analysisRange.start, analysisRange.end);
                 displayStats.avgExpenditureGFit = this._calculateAverageInRange(processedData, 'googleFitTDEE', analysisRange.start, analysisRange.end);
                 displayStats.avgNetBalance = this._calculateAverageInRange(processedData, 'netBalance', analysisRange.start, analysisRange.end);
                 displayStats.avgTDEE_Difference = this._calculateAverageInRange(processedData, 'avgTdeeDifference', analysisRange.start, analysisRange.end);
                 displayStats.avgTDEE_Adaptive = this._calculateAverageInRange(processedData, 'adaptiveTDEE', analysisRange.start, analysisRange.end);
                 displayStats.weightDataConsistency = this._calculateCountInRange(processedData, 'value', analysisRange.start, analysisRange.end);
                 displayStats.calorieDataConsistency = this._calculateCountInRange(processedData, 'calorieIntake', analysisRange.start, analysisRange.end);

                 // Regression
                 if (effectiveRegRange.start && effectiveRegRange.end) { /* ... */
                     const regressionData = processedData.filter(d => d.date instanceof Date && d.date >= effectiveRegRange.start && d.date <= effectiveRegRange.end); // Use processedData for full range
                     results.regressionResult = DataService.calculateLinearRegression(regressionData, effectiveRegRange.start); // Pass start date
                 ...
                 // TDEE from Trend
                 const trendForTDEECalc = displayStats.regressionSlopeWeekly ?? displayStats.currentWeeklyRate; /* ... */
                 displayStats.avgTDEE_WgtChange = this._calculateTDEEFromTrend(displayStats.avgIntake, trendForTDEECalc);
                 displayStats.estimatedDeficitSurplus = this._estimateDeficitSurplusFromTrend(trendForTDEECalc);
                 // Goal Achievement Check
                 const weightThreshold = 0.1; /* ... */
                 if (goal.weight != null) {
                      let achievedPoint = null;
                      for (const d of results.filteredData) {
                           if (d.sma != null && Math.abs(goal.weight - d.sma) <= weightThreshold) { achievedPoint = d; break; }
                      }
        ...
        displayStats.weightToGoal = (referenceWeightForGoal != null && displayStats.targetWeight != null) ? displayStats.targetWeight - referenceWeightForGoal : null;
        ...
        displayStats.requiredRateForGoal = (displayStats.targetDate && !isAchieved) ? this._calculateRequiredRateForGoal(referenceWeightForGoal, displayStats.targetWeight, displayStats.targetDate) : null;
        // Required Calorie Adjustment & Suggested Intake
        displayStats.requiredCalorieAdjustment = null; displayStats.requiredNetCalories = null; displayStats.suggestedIntakeRange = null; /* ... */
        const baselineTDEE = displayStats.avgTDEE_Adaptive ?? displayStats.avgTDEE_WgtChange ?? displayStats.avgExpenditureGFit;
        if (displayStats.targetRate != null && currentTrendForGoal != null && !isNaN(currentTrendForGoal) && !isNaN(displayStats.targetRate) && baselineTDEE != null && !isNaN(baselineTDEE)) { /* ... */
        ...
        if (displayStats.requiredRateForGoal != null && baselineTDEE != null && !isNaN(baselineTDEE) && !isNaN(displayStats.requiredRateForGoal)) { /* ... */
             const requiredDailyDeficitSurplus = (displayStats.requiredRateForGoal / 7) * CONFIG.KCALS_PER_KG;
             displayStats.requiredNetCalories = requiredDailyDeficitSurplus;
             const targetIntake = baselineTDEE + requiredDailyDeficitSurplus;
             if (!isNaN(targetIntake)) { displayStats.suggestedIntakeRange = { min: Math.round(targetIntake - 100), max: Math.round(targetIntake + 100) }; }
        }
        // Target Rate Feedback
        displayStats.targetRateFeedback = { text: "N/A", class: "" }; /* ... */
         if (displayStats.targetRate != null && currentTrendForGoal != null && !isNaN(currentTrendForGoal) && !isNaN(displayStats.targetRate)) { /* ... */
    ...
    update() {
        ...
        try {
            ...
            if (!Selectors.selectProcessedData(stateSnapshot) || Selectors.selectProcessedData(stateSnapshot).length === 0) {
                 console.warn("[StatsManager update] Skipping update - no processed data available.");
                 return;
            }
            ...
            StateManager.dispatch({ type: 'SET_FILTERED_DATA', payload: derivedData.filteredData });
            StateManager.dispatch({ type: 'SET_PLATEAUS', payload: derivedData.plateaus });
            StateManager.dispatch({ type: 'SET_TREND_CHANGES', payload: derivedData.trendChangePoints });
            StateManager.dispatch({ type: 'SET_REGRESSION_RESULT', payload: derivedData.regressionResult });
            StateManager.dispatch({ type: 'SET_WEEKLY_SUMMARY', payload: derivedData.weeklySummaryData });
            StateManager.dispatch({ type: 'SET_CORRELATION_DATA', payload: derivedData.correlationScatterData });
            StateManager.dispatch({ type: 'SET_GOAL_ACHIEVED_DATE', payload: derivedData.goalAchievedDate });
            StateManager.dispatch({ type: 'SET_DISPLAY_STATS', payload: derivedData.displayStats });


        } catch (error) {
            console.error("StatsManager: Error during derived data calculation/update:", error);
    ...
    init() {
      ...
      relevantEvents.forEach(eventName => {
          StateManager.subscribeToSpecificEvent(eventName, () => {
              console.log(`[StatsManager] Received primary event: ${eventName}. Triggering update.`);
